  ...
  describe '#define_page_mappings' do
    context 'mapping includes a literal' do
      it 'creates a matcher to contain the specification' do
        ...
        expect(subject.transitions).to eq(PageMagic::Matcher.new(:path) => :page)
      end
    end
  ...
  describe '#execute_script' do
    ...
    context 'Capybara session does not support #execute_script' do
      ...
      it 'raises an error' do
        ...
        expect { subject.execute_script(:script) }.to raise_error(PageMagic::NotSupportedException, expected_message)
      end
    end
  end
    ...
    context 'base_url has a / on the end' do
      context 'path has / at the beginning' do
        it 'produces compound url' do
          expect(subject.send(:url, base_url.to_s, path)).to eq(expected_url)
        end
      end
    ...
    context 'page supplied' do
      ...
      context 'mapping is a regular expression' do
        it 'raises an error' do
          ...
          expect do
            session.visit(page)
          end.to raise_exception PageMagic::InvalidURLException, described_class::REGEXP_MAPPING_MSG
        end
      end
