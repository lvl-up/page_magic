  ...
  describe '#watch' do
    ...
    context 'element name/ method name supplied' do
      it 'stores the watch instruction' do
        ...
        expect(subject.watcher(:object_id)).to eq(PageMagic::Watcher.new(:object_id))
      end
    end
    ...
    context 'element name and attribute supplied' do
      it 'stores the watch instruction' do
        subject.watch(:my_watcher, :text)
        expect(subject.watcher(:my_watcher)).to eq(PageMagic::Watcher.new(:my_watcher, :text))
      end
    end
    ...
    context 'block supplied' do
      it 'stores the watch instruction' do
        ...
        expect(subject.watcher(:my_watcher)).to eq(PageMagic::Watcher.new(:my_watcher, &block))
      end
    end
    ...
    context 'watcher with the same name added' do
      it 'replaces the watcher' do
        ...
        expect(subject.watchers.first).not_to be(original_watcher)
      end
    end
