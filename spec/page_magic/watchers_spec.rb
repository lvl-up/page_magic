  ...
  describe Watchers do
    ...
    describe '#watch' do
      it 'stores the initial value of the watched element' do
        subject.watch(:my_watcher, :text)
        expect(subject.watchers[:my_watcher].last).to eq(:current_value)
      end

      context 'element name and attribute supplied' do
        it 'stores the watch instruction' do
          subject.watch(:my_watcher, :text)
          expect(subject.watchers[:my_watcher]).to eq(Watcher.new(:text))
        end
      end
      ...
      context 'block supplied' do
        it 'stores the watch instruction' do
          ...
          expect(subject.watchers[:my_watcher]).to eq(Watcher.new(&block))
        end
      end
    end
  end
